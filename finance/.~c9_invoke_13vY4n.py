...
@login_required
def index():
    sqlStr = "SELECT 'Cash' as symbol, '' as name, '' as shares, cash as price, cash as total "
    sqlStr = sqlStr + " FROM users WHERE id = ? "
    
    userId = session["user_id"]
    rows_stock = db.execute(sqlStr, userId)
    """Show portfolio of stocks"""
    return render_template("index.html", rows_stocks = rows_stock)
...
def register():
    ...
    if request.method == "POST":
        ...
        # Ensure username exists and password is correct
        if (len(rows) != 1 ):
            return apology("User not created", 403)
        else:
